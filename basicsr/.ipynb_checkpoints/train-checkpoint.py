    ...
    while current_iter <= total_iters:
        ...
        while train_data is not None:
            ...
            if dist.get_rank() == 0 and current_iter % 2000 == 1: 
                ...
                reblurred_s = vis_results[3].permute(1,2,0).cpu().detach().numpy()
                cv2.imwrite(os.path.join(opt['path']['visualization'], "input_%d.png"%(current_iter)), input_s*255)
                cv2.imwrite(os.path.join(opt['path']['visualization'], "gt_%d.png"%(current_iter)), gt_s*255)
                cv2.imwrite(os.path.join(opt['path']['visualization'], "out_%d.png"%(current_iter)), out_s*255)
                cv2.imwrite(os.path.join(opt['path']['visualization'], "reblurred_%d.png"%(current_iter)), reblurred_s*255)
